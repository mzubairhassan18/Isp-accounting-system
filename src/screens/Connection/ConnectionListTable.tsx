...
} from "features/connection/connectionSlice"; // Assuming you have an interface for the Connection model
import { deleteConnection } from "features/connection/connectionSlice"; // Import your deleteConnection action
import { AppDispatch, RootState } from "/store";
import { CheckCircleOutlined, CloseCircleOutlined } from "@ant-design/icons";
import { getTimeAgoFromDate } from "appUtility";
...
const ConnectionListTable: React.FC<ConnectionListTableProps> = ({
  connections,
}) => {
  ...
  useEffect(() => {
    const formattedConnections = connections.map((connection) => ({
      ...connection,
      date: getTimeAgoFromDate(connection.date), // Convert date to "2 days ago" format
    }));
    setData(formattedConnections);
  }, [connections]);
